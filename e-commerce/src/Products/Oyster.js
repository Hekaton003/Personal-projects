...
const Oyster =()=>{
    function getOyster(item){
        if(item.id===11 || item.id===12){
            return true;
        }
        return false;
    }
    const OysterArray=ProductList.filter(getOyster);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Oyster Perpetual watches</h1>
                <div id='container'>
                    {OysterArray.map((p)=>(<Product data={p}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
