...
const Datejust =()=>{
    function getDatejust(item){
        if(item.id===17){
            return true;
        }
        return false;
    }
    const DatejustArray=ProductList.filter(getDatejust);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Date Just 41 watches</h1>
                <div id='container'>
                    {DatejustArray.map((product)=>(<Product data={product}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
