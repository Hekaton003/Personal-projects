...
const Deepsea=()=>{
    function getDeepsea(item){
        if(item.id===16 || item.id===18){
            return true;
        }
        return false;
    }
    const DeepseaArray=ProductList.filter(getDeepsea);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Deepsea watches</h1>
                <div id='container'>
                    {DeepseaArray.map((product)=>(<Product data={product}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
