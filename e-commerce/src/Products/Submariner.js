...
const Submariner =()=>{
    function getSubmariner(item){
        if(item.id===20){
            return true;
        }
        return false;
    }
    const SubmarinerArray=ProductList.filter(getSubmariner);
    return(
        <div className='Product'>
            <div className='Menu'>
                ...
                <Link to='#'>Submariner</Link>
                <Link to='/Cart'><ShoppingCart size={22} id='shopping'/></Link>
            </div>
            <div id='main'>
                <h1>Submariner watches</h1>
                <div id='container'>
                    {SubmarinerArray.map((p)=>(<Product data={p}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
