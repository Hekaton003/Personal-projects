...
const SkyDweller =()=>{
    function getSkyDweller(item){
        if(item.id===8){
            return true;
        }
        return false;
    }
    const SkyDwellerArray=ProductList.filter(getSkyDweller);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Sky Dweller watches</h1>
                <div id='container'>
                    {SkyDwellerArray.map((p)=>(<Product data={p}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
