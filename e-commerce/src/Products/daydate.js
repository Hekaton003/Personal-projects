...
const DayDate =()=>{
    function getDayDate(item){
        if(item.id>12 && item.id<15){
            return true;
        }
        return false;
    }
    const DayDateArray=ProductList.filter(getDayDate);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Day Date watches</h1>
                <div id='container'>
                    {DayDateArray.map((p)=>(<Product data={p}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
