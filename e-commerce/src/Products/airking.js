...
const Airking =()=>{
    function getAirKingElements(item){
        if(item.id===15){
            return true;
        }
        return false;
    }
    const AirkingArray=ProductList.filter(getAirKingElements);
    return(
        <div className='Product'>
            ...
            <div id='main'>
                <h1>Air King watches</h1>
                <div id='container'>
                    {AirkingArray.map((product)=>(<Product data={product}/>))}
                </div>
                <Menu/>
            </div>
            <footer>
